...
### 1. 既然有 AI，为什么不直接丢链接？
新闻网页可能会遇到反爬、SSL、跳转、动态加载、页面结构复杂等问题，AI 工具不一定能稳定拿到正文。

### 2. 既然新闻网页能打开，为什么不直接复制？
复制出来的内容经常混入广告、推荐新闻、版权块、记者邮箱、站点导航或重复内容，后续还需要人工清理。

### 3. 既然有现成工具，为什么还要自己开发？
我没有找到专门面向韩网新闻正文整理、并能接入本地流程的工具。通用工具能抽取正文，但不够贴合韩网噪声处理、元数据保留和样本回归需求
